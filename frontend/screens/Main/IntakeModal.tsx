...
import { View, Text, TouchableOpacity, Modal, StyleSheet, Alert, ImageBackground, Animated, Image } from 'react-native';
...
const IntakeModal: React.FC<IntakeModalProps> = ({ visible, onClose, sleepId }) => {
    ...
    const saveIntakeRecord = async () => {
        ...
        try {
            ...
            Alert.alert('기록 저장', `카페인 : ${caffeineIntake} 알코올: ${alcoholIntake} `, [
                { text: '확인', onPress: () => navigation.navigate('Sleeping', { sleepId }) },
            ]);
            onClose(); // 모달 닫기
        } catch (error) {
